...
public:
    ...
    void addTblEntry(RoutingEntry &newEntry) {
    ...
    void printTable() {
        ...
        for (int i = 0; i < mytbl.tbl.size(); ++i) {
            cout << mytbl.tbl[i].dstip << " | " << mytbl.tbl[i].nexthop << " | " << mytbl.tbl[i].ip_interface << " | "
                 << mytbl.tbl[i].cost << endl;
        }
    }

    vector<pair<NetInterface, Node *>> &getAdjacentNodes() {
...
static vector<pair<string, BroadcastTableEntry>> routingTableBroadcast;

class RoutingNode : public Node {
    vector<pair<string, BroadcastTableEntry>> receivedRoutingTable;
public:
    void recvMsg();

    void sendMsg();

    void initializeGraph(unordered_map<string, vector<pair<Node *, int>>> &graph);
};
